...
import io.minoro75.genshinhelper.data.assets.ru.items_ru.artifacts_ru.*
import io.minoro75.genshinhelper.data.assets.ru.items_ru.books_ru.*
import io.minoro75.genshinhelper.data.assets.ru.items_ru.boss_items_ru.*
import io.minoro75.genshinhelper.data.assets.ru.items_ru.weapon_resources_ru.*
    ...
    fun getItemLocationRu(itemName: String): Flow<List<HowToObtainItem>?> {
        return flow {
            when (itemName) {
                ...
                "Вечный янтарь" -> emit(itemLocationAdapter.fromJson(EveramberDataSourceRu.sourcesList))
    ...
    private fun getCharacterDetailsRu(characterName: String): Flow<CharacterDetails?> {
        return flow {
            when (characterName) {
                ...
                "Мика" -> emit(characterDetailsAdapter.fromJson(MikaDetailsSourceRu.details))
                ...
                "Дэхья" -> emit(characterDetailsAdapter.fromJson(DehyaDetailsSourceRu.detailsRu))
                ...
                "Кирара"-> emit(characterDetailsAdapter.fromJson(KiraraDetailSourceRu.details))
                ...
                "Путешественник Гидро" -> emit(characterDetailsAdapter.fromJson(TravelerHydroDetailsSourceRu.details))

                "Лини" -> emit(characterDetailsAdapter.fromJson(LyneyDetailsSourceRu.details))
